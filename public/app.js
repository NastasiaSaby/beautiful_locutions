...
buf.push("<tr" + (jade.attr("id", entry.id, true, false)) + (jade.cls([injected ? 'injected' : null], [true])) + "><td><div class=\"lead\"><a" + (jade.attr("href", "/entries/" + (entry.id) + "", true, false)) + ">" + (jade.escape(null == (jade_interp = entry.title || entry.url) ? "" : jade_interp)) + "<small class=\"text-muted\"> Par <i>" + (jade.escape((jade_interp = entry.author) == null ? '' : jade_interp)) + "</i> " + (jade.escape((jade_interp = formatDate(entry)) == null ? '' : jade_interp)) + "</small></a></div><p><i>" + (jade.escape(null == (jade_interp = entry.expression) ? "" : jade_interp)) + "</i></p><p>Balancé par " + (jade.escape((jade_interp = (entry.poster || {}).name) == null ? '' : jade_interp)) + "</p><p>");
// iterate (entry.tags || [])
;(function(){
  var $$obj = (entry.tags || []);
